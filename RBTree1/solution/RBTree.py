...
class RBTreeNode:
    ...
    def count_colored_nodes(
            self,
            color: 'RBTreeNode.Color',
            node: 'RBTreeNode' = None
            ) -> int:
        """
        ...
        left_count = self.left.count_colored_nodes(color) if self.left is not None else 0
        right_count = self.right.count_colored_nodes(color) if self.right is not None else 0

        return max([left_count, right_count]) + (1 if self.color == color else 0)
...
class RBTree:
    ...
    def check_red_black_properties(self, node: RBTreeNode = None) -> bool:
        ...
        left_black_count = node.left.count_colored_nodes(RBTreeNode.Color.BLACK)
        right_black_count = node.right.count_colored_nodes(RBTreeNode.Color.BLACK)
    ...
    def visualize(self) -> None:
        ...
        def callback(parent, node, level, pos, left, right):
            print(" " * (level * 4) + ("L-- " if left else "R-- " if right else "") +
...
def main():
    ...
    for i in [10, 5, 20, 15, 30, 25, 40, 35, 50]:
        ...
        if not check:
            ...
            print("Tree does not satisfy red-black properties after inserting", i)
            return
